...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    const khachHangId =
      body.khach_hang_id ?? existing.khach_hang_id ?? await khachHangIdFromHopDong(hopDongId);
    const hasChiTiet = Array.isArray(body.chi_tiet);
    const giaTriGhiNo = hasChiTiet
      ? body.chi_tiet.length
        ? await calcGiaTriGhiNoFromChiTiet(body.chi_tiet, hopDongId)
        : 0
      : hasOwn(body, 'gia_tri_ghi_no')
        ? body.gia_tri_ghi_no ?? 0
        : existing.gia_tri_ghi_no ?? 0;
    ...
    await query(
      `UPDATE phieu_giao_hang
       ...
       WHERE id=?`,
      [
        hasOwn(body, 'so_phieu') ? body.so_phieu : existing.so_phieu,
        hasOwn(body, 'ngay_giao') ? body.ngay_giao : existing.ngay_giao,
        khachHangId,
        hopDongId || null,
        giaTriGhiNo,
        hasOwn(body, 'noi_dung') ? body.noi_dung || '' : existing.noi_dung ?? '',
        hasOwn(body, 'nguoi_tao') ? body.nguoi_tao || '' : existing.nguoi_tao ?? '',
        id,
      ]
    );
