...
import { hasOwnField, mergeMissingFields } from '../utils/patchMerge.js';
...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    const merged = mergeMissingFields(existing, body, [
      'so_phieu',
      'ngay_giao',
      'khach_hang_id',
      'hop_dong_id',
    ...
    const khachHangId =
      merged.khach_hang_id || await khachHangIdFromHopDong(merged.hop_dong_id);
    const giaTriGhiNo = hasOwnField(body, 'chi_tiet')
      ? (body.chi_tiet?.length
          ? await calcGiaTriGhiNoFromChiTiet(body.chi_tiet, merged.hop_dong_id)
          : 0)
