...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'Không tìm thấy phiếu giao hàng' });
    ...
    if (!khachHangId) khachHangId = await khachHangIdFromHopDong(hopDongId);
    ...
    await query(
      ...
      [
        ...
        body.ngay_giao ?? existing.ngay_giao,
        khachHangId,
        hopDongId || null,
        giaTriGhiNo,
        body.noi_dung !== undefined ? (body.noi_dung || '') : (existing.noi_dung || ''),
        body.nguoi_tao !== undefined ? (body.nguoi_tao || '') : (existing.nguoi_tao || ''),
        id,
      ]
    );
