...
export function mergePhieuGiaoHangUpdate(existing, body, khachHangId, hopDongId, giaTriGhiNo) {
  return {
    so_phieu: valueOrExisting(body, existing, 'so_phieu'),
    ngay_giao: valueOrExisting(body, existing, 'ngay_giao'),
...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    const hopDongId = valueOrExisting(body, existing, 'hop_dong_id');
    if (!hopDongId) {
      return res.status(400).json({ error: 'Phiếu giao hàng phải liên kết với hợp đồng' });
    }
    const requestedKhachHangId = valueOrExisting(body, existing, 'khach_hang_id');
    const khachHangId = requestedKhachHangId || await khachHangIdFromHopDong(hopDongId);
    const giaTriGhiNo = hasOwn(body, 'chi_tiet')
      ? (body.chi_tiet?.length ? await calcGiaTriGhiNoFromChiTiet(body.chi_tiet, hopDongId) : 0)
      : (hasOwn(body, 'gia_tri_ghi_no') ? body.gia_tri_ghi_no : existing.gia_tri_ghi_no);
    const update = mergePhieuGiaoHangUpdate(existing, body, khachHangId, hopDongId, giaTriGhiNo);
