...
export function buildPhieuGiaoHangUpdateValues(body, existing, khachHangId, hopDongId, giaTriGhiNo) {
  return [
    valueOrExisting(body, existing, 'so_phieu'),
    valueOrExisting(body, existing, 'ngay_giao'),
    khachHangId,
    hopDongId || null,
    giaTriGhiNo,
    valueOrExisting(body, existing, 'noi_dung') || '',
    valueOrExisting(body, existing, 'nguoi_tao') || '',
  ];
}
...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    const khachHangId =
      valueOrExisting(body, existing, 'khach_hang_id') || await khachHangIdFromHopDong(hopDongId);
    const hasChiTiet = Object.prototype.hasOwnProperty.call(body, 'chi_tiet');
    const giaTriGhiNo = hasChiTiet
    ...
    await query(
      `UPDATE phieu_giao_hang
       ...
       WHERE id=?`,
      [...buildPhieuGiaoHangUpdateValues(body, existing, khachHangId, hopDongId, giaTriGhiNo), id]
    );
