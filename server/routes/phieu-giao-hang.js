...
export function buildPhieuGiaoHangUpdateParams(existing, body, { khachHangId, hopDongId, giaTriGhiNo, id }) {
  return [
    fieldOrExisting(body, existing, 'so_phieu'),
    fieldOrExisting(body, existing, 'ngay_giao'),
    khachHangId,
    hopDongId || null,
    giaTriGhiNo,
    (hasField(body, 'noi_dung') ? body.noi_dung : existing.noi_dung) || '',
    (hasField(body, 'nguoi_tao') ? body.nguoi_tao : existing.nguoi_tao) || '',
    id,
  ];
}
...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    const hopDongChanged =
      hasField(body, 'hop_dong_id') &&
      String(body.hop_dong_id || '') !== String(existing.hop_dong_id || '');
    const khachHangId = hasField(body, 'khach_hang_id')
      ? body.khach_hang_id || await khachHangIdFromHopDong(hopDongId)
      : hopDongChanged
        ? await khachHangIdFromHopDong(hopDongId)
        : existing.khach_hang_id || await khachHangIdFromHopDong(hopDongId);
    const giaTriGhiNo = hasField(body, 'chi_tiet')
    ...
    await query(
      `UPDATE phieu_giao_hang
       ...
       WHERE id=?`,
      buildPhieuGiaoHangUpdateParams(existing, body, {
        khachHangId,
        giaTriGhiNo,
        hopDongId,
        id,
