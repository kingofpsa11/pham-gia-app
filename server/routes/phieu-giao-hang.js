...
export function mergePhieuGiaoHangUpdate(existing, body) {
  return {
    ...body,
    so_phieu: hasOwn(body, 'so_phieu') ? body.so_phieu : existing.so_phieu,
    ngay_giao: hasOwn(body, 'ngay_giao') ? body.ngay_giao : existing.ngay_giao,
    khach_hang_id: hasOwn(body, 'khach_hang_id') ? body.khach_hang_id : existing.khach_hang_id,
    hop_dong_id: hasOwn(body, 'hop_dong_id') ? body.hop_dong_id : existing.hop_dong_id,
    gia_tri_ghi_no: hasOwn(body, 'gia_tri_ghi_no')
      ? body.gia_tri_ghi_no
...
router.put('/phieu-giao-hang/:id', async (req, res) => {
  try {
    ...
    const body = mergePhieuGiaoHangUpdate(existing, req.body || {});
    const chiTiet = Array.isArray(req.body?.chi_tiet) ? req.body.chi_tiet : null;
    if (!body.hop_dong_id) {
      return res.status(400).json({ error: 'Phiếu giao hàng phải liên kết với hợp đồng' });
    }
    const khachHangId = body.khach_hang_id || await khachHangIdFromHopDong(body.hop_dong_id);
    const giaTriGhiNo = chiTiet
      ? (chiTiet.length ? await calcGiaTriGhiNoFromChiTiet(chiTiet, body.hop_dong_id) : 0)
      : body.gia_tri_ghi_no;
