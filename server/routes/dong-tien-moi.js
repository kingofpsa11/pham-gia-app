...
import { patchNullable, patchNumber, patchValue } from '../utils/patchMerge.js';
...
function mergeDongTienUpdate(existing, body) {
  const hasNgayGiaoDich = Object.prototype.hasOwnProperty.call(body, 'ngay_giao_dich');
  ...
  return {
    ...
    chieu_tien: patchNullable(body, existing, 'chieu_tien'),
    tai_khoan_tien_id: patchValue(body, existing, 'tai_khoan_tien_id'),
    tai_khoan_nhan_id: patchNullable(body, existing, 'tai_khoan_nhan_id'),
    so_tien: patchNumber(body, existing, 'so_tien'),
    doi_tuong_id: patchNullable(body, existing, 'doi_tuong_id'),
    khach_hang_id: patchNullable(body, existing, 'khach_hang_id'),
    nha_cung_cap_id: patchNullable(body, existing, 'nha_cung_cap_id'),
    hop_dong_id: patchNullable(body, existing, 'hop_dong_id'),
    hop_dong_mua_id: patchNullable(body, existing, 'hop_dong_mua_id'),
    hang_muc_thu_chi_id: patchNullable(body, existing, 'hang_muc_thu_chi_id'),
    mo_ta_giao_dich: patchNullable(body, existing, 'mo_ta_giao_dich'),
    so_tai_khoan_doi_ung: patchNullable(body, existing, 'so_tai_khoan_doi_ung'),
    ten_tai_khoan_doi_ung: patchNullable(body, existing, 'ten_tai_khoan_doi_ung'),
    so_du_sau_giao_dich: patchValue(body, existing, 'so_du_sau_giao_dich'),
    ma_giao_dich_ngan_hang: patchNullable(body, existing, 'ma_giao_dich_ngan_hang'),
    ghi_chu: patchNullable(body, existing, 'ghi_chu'),
    trang_thai: patchValue(body, existing, 'trang_thai') || 'hoan_thanh',
  };
}
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        if (item.id) {
          ...
          const mergedSoTien = Number(merged.so_tien) || 0;
          if (!merged.loai_giao_dich || !merged.tai_khoan_tien_id || mergedSoTien <= 0) {
