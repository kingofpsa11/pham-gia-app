...
function nullableFieldOrExisting(body, existing, key) {
  return hasOwn(body, key) ? body[key] || null : existing[key] ?? null;
}

export function mergeDongTienUpdate(existing, body) {
  const ngayGiaoDichRaw = fieldOrExisting(body, existing, 'ngay_giao_dich');
  return {
    ngay_giao_dich: parseNgayGiaoDich(ngayGiaoDichRaw),
    ngay_hach_toan: parseNgayHachToan(ngayGiaoDichRaw),
    loai_giao_dich: fieldOrExisting(body, existing, 'loai_giao_dich'),
    chieu_tien: nullableFieldOrExisting(body, existing, 'chieu_tien'),
    tai_khoan_tien_id: fieldOrExisting(body, existing, 'tai_khoan_tien_id'),
    tai_khoan_nhan_id: nullableFieldOrExisting(body, existing, 'tai_khoan_nhan_id'),
    so_tien: hasOwn(body, 'so_tien') ? Number(body.so_tien) || 0 : Number(existing.so_tien) || 0,
    doi_tuong_id: nullableFieldOrExisting(body, existing, 'doi_tuong_id'),
    khach_hang_id: nullableFieldOrExisting(body, existing, 'khach_hang_id'),
    nha_cung_cap_id: nullableFieldOrExisting(body, existing, 'nha_cung_cap_id'),
    hop_dong_id: nullableFieldOrExisting(body, existing, 'hop_dong_id'),
    hop_dong_mua_id: nullableFieldOrExisting(body, existing, 'hop_dong_mua_id'),
    hang_muc_thu_chi_id: nullableFieldOrExisting(body, existing, 'hang_muc_thu_chi_id'),
    mo_ta_giao_dich: nullableFieldOrExisting(body, existing, 'mo_ta_giao_dich'),
    so_tai_khoan_doi_ung: nullableFieldOrExisting(body, existing, 'so_tai_khoan_doi_ung'),
    ten_tai_khoan_doi_ung: nullableFieldOrExisting(body, existing, 'ten_tai_khoan_doi_ung'),
    ...
    ghi_chu: nullableFieldOrExisting(body, existing, 'ghi_chu'),
    trang_thai: hasOwn(body, 'trang_thai')
      ? body.trang_thai || 'hoan_thanh'
      : existing.trang_thai || 'hoan_thanh',
  };
}
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        if (item.id) {
          ...
          const merged = mergeDongTienUpdate(existing, item);
          ...
          await query(
            `UPDATE dong_tien_moi SET
              ...
              so_tien=?, doi_tuong_id=?, hang_muc_thu_chi_id=?, mo_ta_giao_dich=?, khach_hang_id=?, nha_cung_cap_id=?,
