...
import { mergeOmittedFields } from '../utils/patchMerge.js';
...
function preserveExistingTimeForDateOnly(rawDate, existingDate) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        if (item.id) {
          ...
          const merged = mergeOmittedFields(item, existing, DONG_TIEN_UPDATE_FIELDS);
          const ngayGiaoDichInput = hasOwn(item, 'ngay_giao_dich')
            ? preserveExistingTimeForDateOnly(merged.ngay_giao_dich, existing.ngay_giao_dich)
            : merged.ngay_giao_dich;
          const ngayGD = parseNgayGiaoDich(ngayGiaoDichInput);
          const ngayHT = hasOwn(item, 'ngay_giao_dich')
            ? parseNgayHachToan(ngayGiaoDichInput)
...
router.put('/dong-tien-moi/:id', async (req, res) => {
  try {
    ...
    const body = mergeOmittedFields(originalBody, existing, DONG_TIEN_UPDATE_FIELDS);
    const ngayGiaoDichInput = hasOwn(originalBody, 'ngay_giao_dich')
      ? preserveExistingTimeForDateOnly(body.ngay_giao_dich, existing.ngay_giao_dich)
      : body.ngay_giao_dich;
    const ngayGD = parseNgayGiaoDich(ngayGiaoDichInput);
    const ngayHT = hasOwn(originalBody, 'ngay_giao_dich')
      ? parseNgayHachToan(ngayGiaoDichInput)
      : body.ngay_hach_toan;
    await query(
